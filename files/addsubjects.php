    ...
    if (isset($_POST['subjectexcel'])) {
        if (empty($_FILES['file']['tmp_name'])) {
            ...
            while (!feof($handle)) {
                ...
                $name = $filesop[1];
                $type = $filesop[2];
                $specialite = $filesop[4];
                if ($code == "" || $code == "Subject Code") {
                    continue;
                }
                $q = mysqli_query(mysqli_connect("localhost", "root", "", "ttms"),
                    "INSERT INTO subjects VALUES ('$code','$name','$type','$specialite')");
            }
        }
    }
    ?>
        ...
        <div class="modal-body" id="EnterSubject">
            ...
            <div style="display:none" id="addSubjectForm">
                <form action="addsubjectFormValidation.php" method="POST">
                    ...
                    <div class="form-group">
                        <label for="subjectdepartment">specialite</label>
                        <select class="form-control" id="subjectdepartment" name="SD">
                            <option selected disabled>Select</option>
                            <option value="reseau">reseau</option>
                            <option value="industurielle">industurielle</option>
                            <option value="fondamentale">fondamentale</option>
                            <option value="intellegence artificielle">intellegence artificielle</option>
                        </select>
                    </div>
